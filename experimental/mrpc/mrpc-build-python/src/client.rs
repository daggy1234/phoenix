...
fn generate_methods<T: Service>(
    ...
    for method in service.methods() {
        ...
        // Generate unary
        let ident = format!("{}", method.name());

        let (request, response) =
            method.request_response_name(proto_path, compile_well_known_types);

        // #let temp_req_str = format!("{}", request);
        // let mut formatted_req_vec = temp_req_str.split(" :: ").collect::<Vec<&str>>();
        // formatted_req_vec.remove(0);

        let method = format!(r#"
    def {}(self, req: {}) -> {}:
        call_id = self.stub.initiate_call()
        self.stub.unary({}, {}, call_id, req)
        "#, ident,request, response ,service_id, func_id);
